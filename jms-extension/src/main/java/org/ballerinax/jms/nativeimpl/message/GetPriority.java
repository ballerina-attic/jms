...
public class GetPriority extends BlockingNativeCallableUnit {

    @Override
    public void execute(Context context) {
    }

    public static Object getPriority(Strand strand, ObjectValue msgObj) {
        Message message = JmsUtils.getJMSMessage(msgObj);
        try {
            return message.getJMSPriority();
        } catch (JMSException e) {
            return BallerinaAdapter.getError("Error when retrieving message priority", e);
        }
    }
