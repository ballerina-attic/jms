...
public class GetMessageID extends BlockingNativeCallableUnit {

    @Override
    public void execute(Context context) {
    }

    public static Object getMessageID(Strand strand, ObjectValue msgObj) {
        Message message = JmsUtils.getJMSMessage(msgObj);
        try {
            return message.getJMSMessageID();
        } catch (JMSException e) {
            return BallerinaAdapter.getError("Error when retrieving message ID", e);
        }
    }
