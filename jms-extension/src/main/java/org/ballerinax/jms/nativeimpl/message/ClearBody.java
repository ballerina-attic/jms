...
public class ClearBody extends BlockingNativeCallableUnit {

    @Override
    public void execute(Context context) {
    }

    public static Object clearBody(Strand strand, ObjectValue msgObj) {

        Message message = JmsUtils.getJMSMessage(msgObj);
        try {
            message.clearBody();
        } catch (JMSException e) {
            return BallerinaAdapter.getError("Error when clearing the message body", e);
        }
        return null;
    }
