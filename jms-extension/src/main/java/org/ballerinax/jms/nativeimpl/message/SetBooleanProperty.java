...
public class SetBooleanProperty extends BlockingNativeCallableUnit {

    @Override
    public void execute(Context context) {
    }

    public Object setBooleanProperty(Strand strand, ObjectValue msgObj, String key, boolean value) {
        Message message = JmsUtils.getJMSMessage(msgObj);
        try {
            message.setBooleanProperty(key, value);
        } catch (JMSException e) {
            return BallerinaAdapter.getError("Error when setting boolean property", e);
        }
        return null;
    }
