...
public class ClearProperties extends BlockingNativeCallableUnit {

    @Override
    public void execute(Context context) {
    }

    public static Object clearProperties(Strand strand, ObjectValue msgObj) {

        Message message = JmsUtils.getJMSMessage(msgObj);
        try {
            message.clearProperties();
        } catch (JMSException e) {
            return BallerinaAdapter.getError("Error when clearing the message properties", e);
        }
        return null;
    }
