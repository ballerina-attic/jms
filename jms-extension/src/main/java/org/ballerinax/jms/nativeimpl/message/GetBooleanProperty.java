...
public class GetBooleanProperty extends BlockingNativeCallableUnit {

    @Override
    public void execute(Context context) {
    }

    public static Object getBooleanProperty(Strand strand, ObjectValue msgObj, String key) {
        Message message = JmsUtils.getJMSMessage(msgObj);
        try {
            return message.getBooleanProperty(key);
        } catch (JMSException e) {
            return BallerinaAdapter.getError("Error when retrieving boolean property", e);
        }
    }
