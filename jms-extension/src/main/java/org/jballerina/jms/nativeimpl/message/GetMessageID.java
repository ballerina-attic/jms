...
public class GetMessageID extends AbstractBlockingAction {
    ...
    public void execute(Context context, CallableUnitCallback callableUnitCallback) {
        ...
        try {
            String stringProperty = message.getJMSMessageID();
            context.setReturnValues(new BString(stringProperty));
        } catch (JMSException e) {
            BallerinaAdapter.returnError("Error when retrieving message ID", context, e);
        }
    }
}
