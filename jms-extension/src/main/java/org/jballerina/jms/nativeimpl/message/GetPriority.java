...
public class GetPriority extends AbstractBlockingAction {
    ...
    public void execute(Context context, CallableUnitCallback callableUnitCallback) {
        ...
        Message message = BallerinaAdapter.getNativeObject(messageStruct,
                                                           ...
                                                           context);
        try {
            long timestamp = message.getJMSPriority();
            context.setReturnValues(new BInteger(timestamp));
        } catch (JMSException e) {
            BallerinaAdapter.returnError("Error when retrieving message priority", context, e);
        }
    }
}
